...
namespace TheGame
{
    public class IceVortex : Spell
    {
        public IceVortex(GameScreen parent, SpellInfo spellInfo, Vector3 target)
        ...
        public override void Initialize()
        {
            ...
            settings.Color = Color.DodgerBlue;
            settings.MaxParticles = 5000;
            settings.Position = target;
            settings.BaseRotation = Quaternion.Identity;
            settings.Scale = 1.0f;
            settings.Texture = GameEngine.Content.Load<Texture2D>("ParticleD");
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (timeElpase > 10)
            {
                ...
                {
                    vortex.AddParticle(new Vector3(r * (float)random.NextDouble() * scale, theta * (float)random.NextDouble(), 0.0f), new Vector3(0.6f, 4.0f, 3.0f), 0.8f, 1.3f, Color.DodgerBlue, null);
                }
            }
        }
    }
