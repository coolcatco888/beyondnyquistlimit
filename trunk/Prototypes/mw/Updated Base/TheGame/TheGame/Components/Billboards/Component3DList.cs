...
namespace TheGame
{
    public class Component3DList : List<Component3D>
    {
        ...
        public List<Component3D> GetShortestChain(Vector3 comparePosition)
        {
            Component3DList copy = (Component3DList)new List<Component3D>(this);
            List<Component3D> chain = new List<Component3D>();

            int index = 0;

            chain.Add(copy.GetNearest(comparePosition));
            copy.Remove(chain[index++]);

            while (copy.Count > 0)
            {
                chain.Add(copy.GetNearest(chain[index].Position));
                copy.Remove(chain[index]);
            }
        ...
        public List<Component3D> GetRandomChain()
        {
            List<Component3D> copy = new List<Component3D>(this);
            List<Component3D> chain = new List<Component3D>();
            ...
            while (count > 0)
            {
                chain.Add(copy[GameEngine.Random.Next(--count)]);
                copy.Remove(chain[index++]);
            }
