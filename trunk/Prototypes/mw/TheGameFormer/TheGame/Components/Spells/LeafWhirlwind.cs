...
namespace TheGame
{
    public class LeafWhirlwind : Spell
    {
        ...
        public override void Initialize()
        {
            ...
            settings.MaxPointCount = 200;
            settings.ParticleDuration = 1.3f;
            settings.PointSpriteSize = 0.6f;
            settings.Position = target;
            settings.Rotation = Quaternion.Identity;
            settings.Scale = 1.0f;
            settings.SpriteTexture = GameEngine.Content.Load<Texture2D>("ParticleE");
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (timeElpase > 10)
            {
                ...
                {
                    vortex.AddParticle(new Vector3(r * (float)random.NextDouble() * scale, theta * (float)random.NextDouble(), 1.0f + (float)random.NextDouble()), new Vector3(0.6f, 4.0f, 0.0f));
                }
            }
        }
    }
